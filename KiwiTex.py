...
class LatexConverter(QThread):
    ...
    def run(self):
        try:
            ...
            self.message.emit("Verificando instalación de MiKTeX o pdflatex...")
            self.progress.emit(10)
            
            # Verificar si pdflatex está disponible en el PATH o en ubicaciones portables
            pdflatex_path = self.find_pdflatex()
            
            if not pdflatex_path:
                self.logger.warning("pdflatex no encontrado, intentando instalar MiKTeX...")
                self.message.emit("pdflatex no encontrado. Intentando instalación automática de MiKTeX...")
                self.progress.emit(20)
                
                # Intentar instalar MiKTeX
                if not self.install_miktex():
                    error_msg = "No se pudo encontrar ni instalar pdflatex automáticamente."
                    self.logger.error(error_msg)
                    self.error.emit("No se pudo encontrar ni instalar pdflatex. Por favor instale MiKTeX manualmente.")
                    return
                else:
                    # Después de instalar, verificar nuevamente
                    pdflatex_path = self.find_pdflatex()
                    if not pdflatex_path:
                        self.error.emit("No se pudo encontrar pdflatex después de la instalación.")
                        return
            
            # Si llegamos aquí, ya sea que MiKTeX ya esté instalado o se haya instalado correctamente
    ...
    def find_pdflatex(self):
        """Buscar pdflatex en ubicaciones comunes y en el PATH"""
        # Verificar en el PATH primero
        try:
            import shutil
            pdflatex_path = shutil.which('pdflatex')
            if pdflatex_path:
                self.logger.debug(f"pdflatex encontrado en PATH: {pdflatex_path}")
                return pdflatex_path
        except Exception as e:
            self.logger.warning(f"Error al buscar pdflatex en PATH: {str(e)}")
        
        # Verificar en ubicaciones estándar de instalación
        standard_paths = [
            Path("miktex-portable\\texmfs\\install\\miktex\\bin\\x64\\pdflatex.exe"),
            Path("miktex-portable\\texmfs\\install\\miktex\\bin\\x86\\pdflatex.exe"),
            Path("C:\\Program Files\\MiKTeX\\miktex\\bin\\x64\\pdflatex.exe"),
            Path("C:\\Program Files\\MiKTeX\\miktex\\bin\\x86\\pdflatex.exe"),
            Path("C:\\Program Files\\MiKTeX 2.9\\miktex\\bin\\x64\\pdflatex.exe"),
            Path("C:\\Program Files\\MiKTeX 2.9\\miktex\\bin\\x86\\pdflatex.exe"),
        ]
        
        # Buscar en el directorio de la aplicación
        app_dir = Path(__file__).parent
        for path in standard_paths:
            # Intentar rutas relativas al directorio de la aplicación
            abs_path = app_dir / path
            if abs_path.exists():
                self.logger.debug(f"pdflatex encontrado en: {abs_path}")
                return str(abs_path)
            # También verificar la ruta absoluta directa
            if path.exists():
                self.logger.debug(f"pdflatex encontrado en: {path}")
                return str(path)
                
        self.logger.warning("No se encontró pdflatex en ninguna ubicación estándar")
        return None
        
    def check_miktex(self) -> bool:
        """Verificar si MiKTeX está instalado en ubicaciones comunes"""
        return self.find_pdflatex() is not None

    def install_miktex(self) -> bool:
        """Intentar instalar MiKTeX desde el instalador local o sugerir instalación manual"""
        try:
            self.logger.info("Intentando instalar MiKTeX")
            
            # Buscar el instalador en varias ubicaciones posibles
            possible_paths = [
                Path(__file__).parent / "requirements" / "basic-miktex-24.1-x64.exe",
                Path("basic-miktex-24.1-x64.exe"),
                Path("miktex-portable\\basic-miktex-24.1-x64.exe"),
                Path("miktex-portable\\basic-miktex-24.1-x64.exe"),
                Path(__file__).parent.parent / "requirements" / "basic-miktex-24.1-x64.exe"
            ]
            
            miktex_installer = None
            for path in possible_paths:
                abs_path = path.absolute()
                if abs_path.exists():
                    miktex_installer = abs_path
                    self.logger.info(f"Instalador encontrado en: {miktex_installer}")
                    break
            
            if not miktex_installer:
                self.logger.warning("No se encontró el instalador de MiKTeX en ninguna ubicación esperada")
                self.message.emit("No se encontró el instalador de MiKTeX. Verifique que el archivo 'basic-miktex-24.1-x64.exe' esté en la carpeta 'requirements'.")
                return False
                
            self.logger.info(f"Ejecutando instalador: {miktex_installer}")
            self.message.emit("Instalando MiKTeX (esto puede tomar varios minutos. Por favor espere)...")
            
            try:
                # Crear directorio de instalación portátil si no existe
                portable_dir = Path("miktex-portable")
                portable_dir.mkdir(exist_ok=True)
                
                # Instalar en modo portátil
                cmd = [
                    str(miktex_installer),
                    "--unattended",
                    "--shared",
                    f"--portable={portable_dir}",
                    "--auto-install=yes"
                ]
                
                self.logger.info(f"Comando de instalación: {' '.join(cmd)}")
                ...
                if result.returncode != 0:
                    error_msg = f"La instalación de MiKTeX falló con código {result.returncode}. Error: {result.stderr}"
                    self.logger.error(error_msg)
                    self.message.emit(f"Error durante la instalación: {result.stderr[:200]}...")
                    return False
                
                # Verificar si la instalación fue exitosa
                pdflatex_path = self.find_pdflatex()
                if pdflatex_path:
                    self.logger.info(f"Instalación de MiKTeX completada exitosamente en: {pdflatex_path}")
                    self.message.emit("MiKTeX instalado correctamente en modo portátil.")
                    return True
                else:
                    self.logger.error("No se pudo verificar la instalación de pdflatex después de la instalación")
                    self.message.emit("La instalación de MiKTeX no se completó correctamente.")
                    return False
                
            except subprocess.TimeoutExpired:
                self.logger.error("Tiempo de espera agotado durante la instalación de MiKTeX")
                self.message.emit("La instalación de MiKTeX está tomando más tiempo de lo esperado. Por favor espere...")
                return False
                
        except Exception as e:
            error_msg = f"Error durante la instalación de MiKTeX: {str(e)}"
            self.logger.error(error_msg, exc_info=True)
            self.message.emit(f"Error durante la instalación: {str(e)[:200]}...")
            return False
    ...
    def convert_to_pdf(self):
        try:
            ...
            # Obtener la ruta de pdflatex
            pdflatex_path = self.find_pdflatex()
            if not pdflatex_path:
                error_msg = "No se pudo encontrar pdflatex. Verifique la instalación de MiKTeX."
                self.logger.error(error_msg)
            ...
            with open(bat_file, 'w', encoding='utf-8') as f:
                f.write(f'"{pdflatex_path}" -interaction=nonstopmode -output-directory="{output_dir}" "{tex_file}"')
            
            # Ejecutar el archivo .bat
            self.logger.info(f"Ejecutando: {bat_file}")
            result = subprocess.run(
                [str(bat_file)],
                cwd=output_dir,
                check=False,
            ...
            if result.returncode != 0:
                self.logger.warning(f"pdflatex devolvió código {result.returncode}. Error: {result.stderr}")
                # Continuar a pesar del error, a veces pdflatex puede generar el PDF de todos modos
            ...
            if output_file.exists():
                self.message.emit(f"PDF generado exitosamente en: {output_file}")
                self.progress.emit(100)
                return True
            else:
                error_msg = "No se pudo generar el archivo PDF. Verifique los errores en el archivo de registro."
                if result.stderr:
                    error_msg += f"\n\nError: {result.stderr[-500:]}"  # Mostrar los últimos 500 caracteres del error
                self.logger.error(error_msg)
                self.error.emit(error_msg)
                return False
...
class KiwiTex(QMainWindow):
    def __init__(self):
        ...
        try:
            ...
            icon_paths = [
                Path("kiwi.ico"),
                Path(__file__).parent / "kiwi.ico",
                Path(__file__).parent.parent / "kiwi.ico"
            ]
            
            for icon_path in icon_paths:
                if icon_path.exists():
                    self.setWindowIcon(QIcon(str(icon_path)))
                    self.logger.info(f"Ícono cargado desde: {icon_path}")
                    break
        except Exception as e:
        ...
        # Inicializar UI
        self.init_ui()
        self.output_dir = str(Path.home() / "Documents" / "KiwiTex_Output")
        self.tex_file = None
...
def main():
    ...
    try:
        ...
        # Registrar cualquier error no capturado
        logger = AppLogger("KiwiTex").get_logger()
        logger.critical(f"Error crítico no capturado: {str(e)}", exc_info=True)
        
        # Mostrar mensaje de error al usuario
        error_msg = f"Se produjo un error inesperado: {str(e)}\n\nPor favor, revise el archivo de registro para más detalles."
